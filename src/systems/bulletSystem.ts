...
// Precompute a single rotation table (60 directions) to avoid per-spawn trig cost.
const ANGLE_COUNT = 60;
const ANGLE_VECS: Array<{cx:number; sy:number}> = Array.from({ length: ANGLE_COUNT }, (_, i) => {
...
export function createBulletSystem(state: GameState): System {
  return {
    ...
    update: (dt, _ctx: OrchestratorContext) => {
      const GOLDEN_MODE = !!process.env.GOLDEN_MODE; // freeze current tuning when true
      const debugPhases = process.env.PERF_DEBUG_PHASES;
      let tSpawnStart:number|undefined; let spawnCount=0; let integrateStart:number|undefined;
      if (debugPhases) tSpawnStart = performance.now();
      state.bulletTimer += dt;
      // Fire interval: stable baseline captured under GOLDEN_MODE; future balance tweaks go in else branch.
      let fireInterval: number;
      if (GOLDEN_MODE) {
        fireInterval = 0.2;
        if (state.overdriveActive) fireInterval *= 0.5;
      } else {
        fireInterval = 0.2;
        if (state.overdriveActive) fireInterval *= 0.5;
      }
      if (state.bulletTimer >= fireInterval) {
        // Determine how many bullets to spawn this frame (usually 0 or 1, occasionally >1 if frame longer)
        ...
        for (let i = 0; i < spawnN; i++) {
          ...
          if (!slot) break; // pool exhaustion safety
          const idx = state.nextBulletId++;
          const v = ANGLE_VECS[idx % ANGLE_COUNT];
          slot.id = idx;
          slot.x = player.x; slot.y = player.y;
          slot.vx = v.cx * speed; slot.vy = v.sy * speed;
          slot.alive = true;
          state.bullets.push(slot as Bullet);
          spawnCount++;
        }
      }
      if (debugPhases) integrateStart = performance.now();
      // Manual index loop can be a tad faster than for..of with condition branch inside
      for (let i=0;i<state.bullets.length;i++) {
        const b = state.bullets[i];
        if (!b.alive) continue;
        b.x += b.vx * dt; b.y += b.vy * dt;
    // Boss collision (simple circle) – frozen behavior in golden mode (damage=12)
    if (state.bossHealth > 0) {
          const hb = state.bossHitbox; const dx = b.x - hb.x; const dy = b.y - hb.y; const r = hb.radius;
            if (dx*dx + dy*dy <= r*r) {
      const dmg = GOLDEN_MODE ? 12 : 12; // adjust outside golden mode if balance changes needed
              state.bossHealth = Math.max(0, state.bossHealth - dmg);
              const log = state.bossDamageLog; log.push({ frame: (globalThis as any).__frame ?? 0, amount: dmg, source: 'playerBullet', hpAfter: state.bossHealth });
              if (log.length > 100) log.splice(0, log.length - 100);
              b.alive = false;
              // fast removal swap
              const last = state.bullets[state.bullets.length - 1];
              state.bullets[i] = last; state.bullets.pop(); i--; // adjust loop
